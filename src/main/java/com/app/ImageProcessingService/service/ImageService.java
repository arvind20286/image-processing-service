...
import com.app.ImageProcessingService.repos.ImagesRepository;
import ij.IJ;
import ij.ImagePlus;
...
//        File tempFile = File.createTempFile("image", "." + image.getFileExtension());
        Path destination = Paths.get("/Users/arvindkumar/Documents/ImageProcessingService/image." + image.getFileExtension());
        try {
            log.info("Attempting to download image from S3: bucket={}, key={}", bucketName, image.getFileName());
            s3Client.getObject(GetObjectRequest.builder().bucket(bucketName).key(image.getFileName()).build(), ResponseTransformer.toFile(destination));
            log.info("Successfully downloaded image from S3 to {}", destination.toString());
        } catch (Exception e) {
        ...
        log.info(image.getFileExtension());
        // Save the transformed image
        String transformedFileName = UUID.randomUUID().toString() + "." + (format != null ? format : image.getFileExtension());
        File transformedFile = new File(destination.getParent().toString(), transformedFileName);
//        IJ.saveAs(new ImagePlus("", processor), image.getFileExtension(), transformedFile.getAbsolutePath());
        BufferedImage transformedImageObject = processor.getBufferedImage();
        try {
            ImageIO.write(transformedImageObject, image.getFileExtension(), transformedFile);
            System.out.println("Image saved as: " + transformedFile.getAbsolutePath());
        } catch (Exception e) {
            e.printStackTrace();
        }
        ...
        PutObjectRequest putObjectRequest = PutObjectRequest.builder()
                ...
                .build();
        s3Client.putObject(putObjectRequest, RequestBody.fromFile(transformedFile));
        ...
        File fileToDelete = new File(destination.toString());

        if (fileToDelete.delete()) {
            System.out.println("File deleted successfully: " + destination);
        } else {
            System.out.println("Failed to delete the file: " + destination);
        }
